...
- **健康检查**：`/healthz` 端点 + `HealthHandler()`，k8s 存活/就绪探针开箱即用
...
Keyed 汇聚（D-26）、生命周期钩子、背压可视化（D-27）、拓扑校验（D-28）、内置限流器（D-29）、MergeNode 可观测性（D-30）、重试退避（D-31）、默认路由分支（D-32）、链路汇总指标（D-33）、输出收集与信号（D-34）、路由流量观测（D-35）——128 个测试（含集成/并发/泄漏）全部通过，
