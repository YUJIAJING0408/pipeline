...
| D-22 慢处理计时（+1.1.4 结构化） | `StageConfig.SlowThreshold` + workerPool 计时 | 每个 Stage 的 `StageConfig` 增加 `SlowThreshold time.Duration`（慢处理阈值，0 表示不启用）。workerPool 对每条 process 计时，耗时**超过阈值即打印慢日志**（`[slow] stage=%s input=%v took=%v threshold=%v`），不区分成功/失败（失败同样提示，便于诊断）。1.1.4 起从 `fmt.Printf` stdout 改为 `StageLogger.Warnw` 结构化日志，落盘到 `{stage}.log`，轮转/采样/采集全覆盖；MergeNode 对齐（D-30） |
